...
VC_results = read.csv(here::here("results", "VCSC_PVS_results.csv")) %>%
  select(val, lower, upper, group, inter, scenario)
CP_care_results = read.csv(here::here("results", "CPRandCare_PVS_results.csv")) %>%
  select(val, lower, upper, group, inter, scenario)
...
CP_all_results = read.csv(here::here("results", "CPRandAll_PVS_results.csv")) %>%
  select(val, lower, upper, group, inter, scenario)
VC_care_results = read.csv(here::here("results", "VCRandCare_PVS_results.csv")) %>%
  select(val, lower, upper, group, inter, scenario)
...
ggsave(here::here("figures", "suppfig5.png"), width = 13, height = 7)
