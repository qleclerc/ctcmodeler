...
CP_results = read.csv(here::here("results", "CPSC_60_results_alt.csv")) %>%
  select(val, lower, upper, group, inter, scenario)
VC_results = read.csv(here::here("results", "VCSC_60_results_alt.csv")) %>%
  select(val, lower, upper, group, inter, scenario)
CP_care_results = read.csv(here::here("results", "CPRandCare_60_results_alt.csv")) %>%
  select(val, lower, upper, group, inter, scenario)
CP_pat_results = read.csv(here::here("results", "CPRandPat_60_results_alt.csv")) %>%
  select(val, lower, upper, group, inter, scenario)
CP_all_results = read.csv(here::here("results", "CPRandAll_60_results_alt.csv")) %>%
  select(val, lower, upper, group, inter, scenario)
VC_care_results = read.csv(here::here("results", "VCRandCare_60_results_alt.csv")) %>%
  select(val, lower, upper, group, inter, scenario)
VC_pat_results = read.csv(here::here("results", "VCRandPat_60_results_alt.csv")) %>%
  select(val, lower, upper, group, inter, scenario)
VC_all_results = read.csv(here::here("results", "VCRandAll_60_results_alt.csv")) %>%
  select(val, lower, upper, group, inter, scenario)
...
ggplot(compare2_results,
  ...
  coord_cartesian(ylim = c(-0.01, 0.30), clip = "off") +
  theme_bw() +
...
ggsave(here::here("figures", "fig5_pat.png"), width = 13, height = 7)
